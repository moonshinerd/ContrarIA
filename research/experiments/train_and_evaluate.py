"""Treinamento, avaliação de mudança de domínio (texto completo vs truncado) e benchmark de latência.
...
e salva o melhor modelo exportado em joblib.
"""
...
BLUESKY_SAMPLE = [
    # Fakes
    ("URGENTE: Urnas eletrônicas foram apreendidas pela PF com código adulterado para alterar votos!", 1),
    ("BOMBA: TSE confirma que houve inserção de 5 milhões de votos fantasmas na última eleição.", 1),
    ("Comprovado: vacinas contra covid contêm grafeno para monitoramento populacional via 5G.", 1),
    ("Governo decreta confisco de todas as contas poupança a partir da próxima segunda-feira.", 1),
    ("Vídeo mostra ministros do STF em reunião secreta fraudando ata judicial em hotel de luxo.", 1),
    ("ONU aprova resolução autorizando intervenção militar imediata em território brasileiro.", 1),
    ("Lula assina medida provisória proibindo o cristianismo e fechando igrejas evangélicas.", 1),
    ("Descobriram plano secreto do Foro de SP para unificar moedas da América do Sul semana que vem.", 1),
    ("Anvisa escondeu laudo que comprovava cura do câncer por remédio caseiro popular.", 1),
    ("Fraude nas urnas do Amazonas: cidade com 10 mil eleitores teve 50 mil votos computados.", 1),
    # Fatos reais
    ("TSE conclui teste público de segurança das urnas com participação de peritos e universidades.", 0),
    ("Banco Central anuncia aumento de 0,25 ponto percentual na taxa Selic após reunião do Copom.", 0),
    ("Câmara dos Deputados aprova em segundo turno o texto principal da reforma tributária.", 0),
    ("Ministério da Saúde inicia campanha nacional de vacinação contra a gripe em postos de saúde.", 0),
    ("IBGE divulga taxa de desemprego que recua para 6,8% no trimestre encerrado em julho.", 0),
    ("Supremo Tribunal Federal julga ação sobre marco temporal para demarcação de terras indígenas.", 0),
    ("Petrobras anuncia redução no preço do litro da gasolina para as distribuidoras.", 0),
    ("Governo federal envia ao Congresso o projeto da Lei Orçamentária Anual para o próximo ano.", 0),
    ("Polícia Federal deflagra operação contra desmatamento ilegal em terras públicas na Amazônia.", 0),
    ("Congresso Nacional promulga emenda constitucional que amplia repasses ao Fundeb.", 0),
]
...
def prepare_data():
    ...
    # Texto truncado para simular tamanho do Bluesky (título + início da matéria até 300 caracteres)
...
def main():
    ...
    print(f"    [Texto Completo] F1: {m_full['f1']:.4f} | Acurácia: {m_full['accuracy']:.4f} | AUC: {m_full['roc_auc']:.4f}")
    ...
    print(f"    [Texto Truncado - Mudança de Domínio] F1: {m_shift['f1']:.4f} | Acurácia: {m_shift['accuracy']:.4f} | AUC: {m_shift['roc_auc']:.4f}")
    ...
    print(f"    [SVM Completo] F1: {m_svm_full['f1']:.4f} | Acurácia: {m_svm_full['accuracy']:.4f} | AUC: {m_svm_full['roc_auc']:.4f}")

    m_svm_trunc = evaluate_model(pipe_svm, X_test_trunc, y_test, name="SVM_Dominio_Truncado")
    results.append(m_svm_trunc)
    print(f"    [SVM Truncado] F1: {m_svm_trunc['f1']:.4f} | Acurácia: {m_svm_trunc['accuracy']:.4f} | AUC: {m_svm_trunc['roc_auc']:.4f}")

    # Treina modelo especialista em texto curto (truncado)
    print("\n[*] Treinando LogisticRegression treinado diretamente em TEXTO TRUNCADO (<=300 chars)...")
    ...
    print(f"    [LR Treinado em Truncado] F1: {m_adapted['f1']:.4f} | Acurácia: {m_adapted['accuracy']:.4f} | AUC: {m_adapted['roc_auc']:.4f}")
    ...
    print(f"    Latência média: {latency_per_item_ms:.3f} ms/item (Total: {duration_infer:.3f}s para 1000 items)")
    ...
    print(f"    F1: {m_bsky['f1']:.4f} | Acurácia: {m_bsky['accuracy']:.4f} | AUC: {m_bsky['roc_auc']:.4f}")
    ...
    print(f"\n[OK] Modelo exportado (especialista em textos curtos) salvo em: {model_export_path}")
